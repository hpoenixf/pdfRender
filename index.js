...
function pdfRender(url, container = 'body',
                   eachRender = 20,
                   scriptOpt = {hasPdfJSscript: false, scriptSrc: '//mozilla.github.io/pdf.js/build/pdf.js', workerSrc: '//mozilla.github.io/pdf.js/build/pdf.worker.js'}) {
  ...
  let pageHeight
  let renderCount = 1
  if (!scriptOpt.hasPDFJSscript) {
    let PDFJSscript = document.createElement('script')
    // use cdn may be more quickly
    PDFJSscript.src = scriptOpt.scriptSrc
    document.body.appendChild(PDFJSscript)
    PDFJSscript.onload = startRender
  } else {
    startRender()
  }
  ...
  function startRender() {
    PDFJS.workerSrc = scriptOpt.workerSrc
